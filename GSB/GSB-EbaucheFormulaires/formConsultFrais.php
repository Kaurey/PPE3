...
<div name="droite" style="float:left;width:80%;">
	...
	<form name="formConsultFrais" method="post" action="chercheFrais.php">
		<h1> Période </h1>
			<label class="titre">Mois/Année :</label><input type="month" id="start" name="trip-start">
		<p class="titre"/>
		...
		<table style="color:white;" border="1">
			...
			SELECT `lignefraishorsforfait`.`date`, `lignefraishorsforfait`.`libelle`, `lignefraishorsforfait`.`montant`, fichefrais.idEtat, `lignefraishorsforfait`.`mois` FROM `fichefrais` LEFT JOIN `lignefraishorsforfait` ON `lignefraishorsforfait`.`idVisiteur` = `fichefrais`.`idVisiteur`
				*/?>
			</tr>
		</table>
